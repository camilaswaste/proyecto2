...
export default function SocioPerfilPage() {
  ...
  useEffect(() => {
    ...
    if (userData) {
      setUser(userData)
      if (userData.socioID) {
        fetchProfileData(userData.socioID)
      }
    }
  }, [])

  const fetchProfileData = async (socioID: number) => {
    try {
      const response = await fetch(`/api/socio/perfil?socioID=${socioID}`)
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/socio/perfil", {
        method: "PUT",
            ...
            {isEditingProfile ? (
              <form onSubmit={handleUpdateProfile} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="rut" value={formData.rut} disabled className="bg-gray-50" />
                  <p className="text-sm text-gray-500">El RUT no se puede modificar</p>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{profileData?.RUT || "-"}</p>
                </div>
                <div>
